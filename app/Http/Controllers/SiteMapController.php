    ...
    public function index($sitename, $sitedomen)
    {
        ...
        if (!$sitemap->isCached()) {
            ...
            foreach ($pages as $value) {
                try {
                    $sitemap->add('http://' . $getSites->domen . '/' . $value->SlugName . "/" . $value->id,
                        Carbon::now($value->update_at)->toAtomString());
                } catch (\Exception $e) {
                }
            }
            ...
            if ($sitename != 'stomzn48') {
                ...
                foreach ($goods as $value) {
                    try {
                        $sitemap->add('http://' . $getSites->domen . '/' . $value->SlugName . "/" . ($value->slug ? $value->slug : $value->id),
                            $value->updated_at->toAtomString());
                    } catch (\Exception $e) {
                    }
            ...
            foreach ($complex as $value) {
                try {
                    $sitemap->add('http://' . $getSites->domen . '/' . (new Goods())->SlugName . '/' . $value->SlugName . "/" . ($value->slug ? $value->slug : $value->id),
                        $value->updated_at->toAtomString());
                } catch (\Exception $e) {
                }
            }
