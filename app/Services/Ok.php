...
class Ok
{
    ...
    public static function changeCodeToToken($code)
    {
        ...
        curl_setopt($curl, CURLOPT_POSTFIELDS,
            'code=' . $code . '&redirect_uri=' . self::$redirect_url . '&grant_type=authorization_code&client_id=' . self::$app_id . '&client_secret=' . self::$app_secret_key);
    ...
    public static function updateAccessTokenWithRefreshToken()
    {
        ...
        curl_setopt($curl, CURLOPT_POST, true);
        curl_setopt($curl, CURLOPT_POSTFIELDS,
            'refresh_token=' . self::$refresh_token . '&grant_type=refresh_token&client_id=' . self::$app_id . '&client_secret=' . self::$app_secret_key);
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
        $s = curl_exec($curl);
        curl_close($curl);
        $a = json_decode($s, true);
    ...
    public function __consruct($app_id, $app_public_key, $app_secret_key)
    {
        $this->app_id = $app_id;
